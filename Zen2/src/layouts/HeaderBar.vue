    ...
    <div class="container-fluid text-white m-0 p-0" style="background-color: #2f3640">
      <router-link to="/" class="navbar-brand text-white">
        <div class="d-flex align-items-base">
          <mdicon name="library-outline" class="text-warning color-success mx-3" :size="36" />
          <span class="fs-4">ZenLibrary</span>
        </div>
      </router-link>
      <div>
        <button
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
          ...
          <ul class="navbar-nav me-auto mb-3 mb-lg-0 my-auto align-items-center">
            ...
            <li v-if="user && user.role == 'user' && authenticated" class="nav-item">
              <router-link :to="{ name: 'cart' }" class="nav-link">
                <button class="btn btn-sm btn-outline-rosy-brown">
                  <mdicon name="book-open-variant-outline" :size="20" />
                  MyBooks
                  <span class="badge text-bg-warning">{{ request.items.length }}</span>
                </button>
              </router-link>
            </li>
...
import { userequestStore } from '../stores/cartstore'
